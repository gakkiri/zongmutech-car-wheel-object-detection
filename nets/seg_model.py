...
class ResNet(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        return x, low_level_feat  # x:[1, 2048, 32, 32]  low_level_features:[1,256, 128, 128]
...
def Resnet50_FPN(pretrained=False):
    fpn = models.detection.fasterrcnn_resnet50_fpn(pretrained=False, pretrained_backbone=pretrained).backbone
    return fpn
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True, fpn=True):
        if _print:
            print("Constructing DeepLabv3+ model...")
            print("Number of classes       : {}".format(n_classes))
            print("Output stride           : {}".format(os))
            print("Number of Input Channels: {}".format(nInputChannels))
            print("Input shape             : {}".format(f"batchsize, {nInputChannels}, 720, 1280"))
            print("Output shape            : {}".format(f"batchsize, {n_classes}, 720, 1280"))
        ...
        self.fpn = fpn
        if self.fpn:
            self.resnet_features = Resnet50_FPN(pretrained)
            self.conv_x = nn.Conv2d(256, 2048, 1, 1, bias=False)
        else:
    ...
    def forward(self, input):  # input 1, 3, 512, 512
        if self.fpn:
            feats_dict = self.resnet_features(input)
            x, low_level_features = feats_dict[2], feats_dict[0]
            x = self.conv_x(x)
        else:
            x, low_level_features = self.resnet_features(input)  # [1, 2048, 32, 32]  [1,256, 128, 128].
...
if __name__ == "__main__":
    model = DeepLabv3_plus(nInputChannels=3, n_classes=4, os=16, pretrained=True, _print=True)
    model.eval()
