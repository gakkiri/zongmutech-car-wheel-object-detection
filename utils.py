...
final_voc_labels = ('car', 'person', 'truck', 'bus', 'rider', 'rear', 'front')
# voc_labels = ('car', 'person', 'truck', 'bus', 'rider')
voc_labels = ('car', 'person', 'truck', 'bus', 'rider', 'wheel')

label_map = {k: v + 1 for v, k in enumerate(voc_labels)}
final_label_map = {k: v + 1 for v, k in enumerate(final_voc_labels)}
# print(final_label_map)

label_map['background'] = 0
rev_label_map = {v: k for k, v in label_map.items()}  # Inverse mapping
final_rev_label_map = {v: k for k, v in final_label_map.items()}


# Color map for bounding boxes of detected objects from https://sashat.me/2017/01/11/list-of-20-simple-distinct-colors/
distinct_colors = ['#e6194b', '#3cb44b', '#ffe119', '#0082c8', '#f58231', '#911eb4', '#46f0f0', '#f032e6']
label_color_map = {k: distinct_colors[i] for i, k in enumerate(final_label_map.keys())}


def to_submission(pred_dict):
    pred_dict['labels'] = [final_label_map[i] for i in pred_dict['labels']]
    pred_dict['boxes'] = [[round(j) for j in i] for i in pred_dict['boxes']]  # (xmin, ymin, xmax, ymax)

    # to (xmin, ymin, w, h)
    for i, box in enumerate(pred_dict['boxes']):
        pred_dict['boxes'][i] = [box[0], box[1], box[2]-box[0], box[3]-box[1]]
...
def crop(img, xymin, xymax):
    size = img.size()
    crop_img = torch.zeros(size)
    crop_img[xymin[1]-1:xymax[1], xymin[0]-1: xymax[0]] += img[xymin[1]-1:xymax[1], xymin[0]-1: xymax[0]]
...
def get_wheel_mask(file):
    ...
    for object in collection.getElementsByTagName('object'):
        if object.getElementsByTagName('name')[0].childNodes[0].data == 'rear' or \
                object.getElementsByTagName('name')[0].childNodes[0].data == 'front':
...
def get_inf(file, get_diff=False):
    classes = []
    xymin = []
    xymax = []
    if get_diff:
    ...
    for object in collection.getElementsByTagName('object'):
        # if 'rear' not in voc_labels and 'front' not in voc_labels:
        #     if object.getElementsByTagName('name')[0].childNodes[0].data == 'rear' or \
        #             object.getElementsByTagName('name')[0].childNodes[0].data == 'front':
        #         continue

        cls = object.getElementsByTagName('name')[0].childNodes[0].data
        if cls == 'front' or cls == 'rear':
            cls = 'wheel'
        ...
        if get_diff:
            difficulties.append(int(object.getElementsByTagName('difficult')[0].childNodes[0].data))

    if get_diff:
        return classes, xymin, xymax, difficulties
    else:
        return classes, xymin, xymax
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best):
    ...
    root = 'checkpoints/'
    filename = root + 'deeplab_checkpoint.pth.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, root + 'BEST_' + filename)
